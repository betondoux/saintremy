...
def render_essay(slide):
    ...
    base = load_bg(slide).convert('RGBA')
    # 상단 약한 그라디언트 (마스트헤드)
    base.alpha_composite(vgradient(W, 220, top_a=120, bot_a=0, gamma=1.4), (0, 0))

    d_meta = ImageDraw.Draw(base)
    draw_masthead(d_meta, slide.get('kicker', ''), slide.get('series', ''))

    f_title = font(F_BLACK, 50)
    f_lead = font(F_BOLD, 28)
    ...
    lead_block_h = len(lead_lines) * 40 + (24 if lead_lines else 0)

    body_lines = wrap(body, f_body, W - MARGIN * 2, d_meta) if body else []
    body_block_h = len(body_lines) * 38 + (22 if body_lines else 0)

    text_block_h = title_block_h + lead_block_h + body_block_h
    y_top = H - MARGIN - 30 - text_block_h
    y_top = max(y_top, 280)

    grad_top = max(y_top - 110, 200)
    grad_h = H - grad_top
    base.alpha_composite(vgradient(W, grad_h, top_a=0, bot_a=225, gamma=1.3), (0, grad_top))
